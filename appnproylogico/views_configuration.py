...
def panel_supervisor(request):
    ...
    try:
        ...
        def _fallback(d):
            ...
            est = (d.estado or '').upper()
            pri = (d.prioridad or '')
            lid = (d.farmacia_origen_local_id or '').strip()
            farm = Localfarmacia.objects.filter(local_id=lid).first()
            cierre_t = getattr(farm, 'funcionamiento_hora_cierre', None)
            now_t = tz.now().time()
            sug = 'Reasignar/Reenviar' if est == 'FALLIDO' else 'Confirmar entrega/Acciones'
            if cierre_t and now_t >= cierre_t and (pri or '').upper() != 'ALTA':
                sug = 'Postergar'
        ...
        for d in qs:
            ...
            rows.append({
                ...
                'sugerencia': out.split('\n')[-1],
            })
        context['ia_sugerencias_preview'] = rows
    except Exception:
        context['ia_sugerencias_preview'] = []
        ...
        if total_actual < 50:
            ...
            for m in motoristas[:50]:
                try:
                    ...
                    if not f:
                        break
                    exists = AsignacionMotoristaFarmacia.objects.filter(motorista=m, farmacia=f, activa=1).exists()
                    if exists:
                        idx_f += 1
                        continue
